...
## 5. Gotcha: `enable_adjacent` is off by default
...
a file in e.g. `~/Downloads` doesn't scan every subdirectory beneath it. The SUPER+I
gallery keybind turns it on for that launch only, via `-e`:

```
swayimg -g -e 'swayimg.imagelist.enable_recursive(true)' $HOME/Pictures
```

This is needed because `~/Pictures` holds no loose images — everything lives in
`~/Pictures/wallpaper/`. Without recursion the gallery opens empty.

## 6. Gotcha: `set_fix_scale` throws in gallery mode
...
Then: double-click an image in Dolphin (opens floating and centered, not in Chromium);
SUPER+I for the gallery grid; `hyprctl clients` shows `class: swayimg`, `floating: true`,
sized 70% × 75% of the logical screen.
...
`swayimg` from `LINK_CONFIG` / `swayimg.desktop` from `LINK_APPS` in `install.sh`.
